...
```{r}
occurrence %<>% mutate(rightsHolder = "Universiteit Gent") # e.g. "INBO"
```
...
```{r}
occurrence %<>% mutate(datasetID = "https://toComplete")
```

### institutionCode

```{r}
occurrence %<>% mutate(institutionCode = "UniversiteitGent") # e.g. "INBO"
```

### datasetName

```{r}
occurrence %<>% mutate(datasetName = "Florient - a database on wild vascular plants in East Flanders in the beginning of the 19th century") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
